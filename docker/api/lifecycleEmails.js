...
Это бесплатно и без обязательств. Если пойдёт — ИИ-репетитор поможет разобрать слабые темы по шагам, не выдавая готовых ответов.`,
  ...
  // Единственное письмо, которое уходит уже ПОСЛЕ того, как человек увидел пейволл (экран «План
  // подготовки», см. PlanView.tsx), а не до него — раньше с этого момента с бесплатным пользователем
  // никто больше не связывался (разбор воронки 26.09.2026). {темы} — абзац со слабыми темами выпадает
  // целиком, если диагностика прошла без единой слабой темы (см. fillBody).
  plan: {
    title: "Напоминание после диагностики тем, кто не оплатил",
    when: "Через сутки-пять после последней диагностики тем, кто остался на бесплатном тарифе. Один раз.",
    eyebrow: "твой план никуда не делся",
    cta: "Открыть план →",
    ctaPath: "/plan",
    rich: false,
    placeholders: ["имя", "предмет", "темы"],
    footer: FOOTER_ONCE,
    subject: "Твой план по предмету «{предмет}» ждёт",
    bodyText: `Ты уже проходил(а) диагностику по предмету «{предмет}» и видел(а) свой персональный план подготовки — он никуда не делся, вместе с уровнем и слабыми темами.

Есть над чем поработать: {темы}.

На бесплатном тарифе ИИ-репетитор отвечает 3 раза в день — для разбора слабых тем по шагам этого обычно мало. На платных тарифах лимита нет: подсказки по уровням, разбор ошибок по шагам, проверка сочинений по критериям.`,
    summary: false,
  },
  abandoned: {
