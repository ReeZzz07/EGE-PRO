...
// подтверждаем прямым SQL и логинимся за токеном, как и в остальных тестах этого проекта.
async function signup() {
  const email = testEmail();
  const resp = await fetch(`${BASE_URL}/auth/signup`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ email, password: PASSWORD, full_name: "Тест Удаление", age: 17, gender: "m" }),
  });
  ...
  return { id, token: loginJson.access_token };
}
