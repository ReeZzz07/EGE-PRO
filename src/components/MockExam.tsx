...
export default function MockExam({ subject, onFinish, onExit }: { subject: Subject; onFinish: () => void; onExit: () => void }) {
  ...
  // на бесплатном тарифе проверки сочинений нет (см. docker/api/server.js) — включать в пробник
  // часть 2, которую всё равно не проверят, смысла нет, урезаем пробник до части 1
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const part2 = useMemo(() => (essayAllowed ? TASKS.find((t) => t.subject === subject && t.answerType === "essay") : undefined), [subject, tasksVersion, essayAllowed]);
        ...
        <Reveal>
          ...
          {essayAllowed === false && (
            <p className="mt-2 text-[12.5px] leading-relaxed text-ink2">
              Часть 2 (развёрнутый ответ) с проверкой по критериям — на платных тарифах. Пробник ограничен частью 1.
            </p>
          )}
