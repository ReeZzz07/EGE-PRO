...
/** Заголовок/описание вкладки браузера для LegalDoc.tsx — фиксированные, без формы в админке
 *  (см. lib/seo.ts): эти страницы не рассчитаны на трафик из поиска, на них попадают по прямой
 *  ссылке, поэтому и в sitemap.xml их нет, и useDocumentHead ставит им noindex. */
export const LEGAL_SEO: Record<LegalDocKey, { title: string; description: string }> = {
  offer: { title: "Публичная оферта — ЕГЭ·ПРО", description: "Публичная оферта на использование образовательной онлайн-платформы «ЕГЭ·ПРО»." },
  privacy: { title: "Политика конфиденциальности — ЕГЭ·ПРО", description: "Политика обработки персональных данных пользователей платформы «ЕГЭ·ПРО»." },
};
