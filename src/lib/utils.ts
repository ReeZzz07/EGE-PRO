...
export function checkAnswer(given: string, accepted: string[]): boolean {
  ...
  return accepted.some((a) => {
    ...
    // и только когда эталонный ответ вообще состоит из набора цифр — иначе это же "снятие
    // разделителей" ошибочно засчитывает верным текстовый ответ из нескольких слов, написанный
    // слитно (например, «клеточная инженерия» ≡ «клеточнаяинженерия»), что не связано с исходной
    // задачей (~2100 текстовых заданий в банке содержат пробел в эталонном ответе).
