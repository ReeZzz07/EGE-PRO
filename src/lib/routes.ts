...
//
// Сознательно без react-router (он есть в зависимостях, но не подключён): здесь нет ни одного
// place, где нужен настоящий роутинг/матчинг — весь рендер по-прежнему один большой switch по
// view.name в App.tsx. History API (pushState + popstate) для двух десятков строк логичнее, чем
// тащить <BrowserRouter>/<Routes> ради несуществующего в проекте паттерна.
