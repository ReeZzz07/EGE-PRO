...
4. **Код и секреты**
   ...
   - `SITE_HOST` — тот же домен, но БЕЗ схемы (`ege-tutor.ru`, не `https://ege-tutor.ru`) — читает
     Caddy, ему для `www.{$SITE_HOST}` в docker/Caddyfile схема мешала бы. Заодно убедись, что A-запись
     `www`-поддомена тоже указывает на IP сервера — Caddy выпускает сертификат и на него;
